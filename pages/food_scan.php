  ...
  <form method="post" enctype="multipart/form-data" class="space-y-4 rounded-xl border border-neutral-800 bg-neutral-900 p-4">
    ...
    <div>
      ...
      <div class="mt-2 flex items-center gap-2">
        <button type="button" id="btnDetect" class="px-3 py-2 rounded-lg bg-neutral-800 hover:bg-neutral-700">Auto-detect from photo</button>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
      <div>
        ...
        <input type="number" name="calories" min="0" step="1" required class="w-full bg-neutral-900 border border-neutral-800 rounded px-3 py-2" />
      </div>
      <div>
        <label class="block text-sm text-neutral-300 mb-1">Protein (g)</label>
        <input type="number" step="0.1" min="0" required name="protein_g" class="w-full bg-neutral-900 border border-neutral-800 rounded px-3 py-2" />
      </div>
      <div>
        <label class="block text-sm text-neutral-300 mb-1">Carbs (g)</label>
        <input type="number" step="0.1" min="0" required name="carbs_g" class="w-full bg-neutral-900 border border-neutral-800 rounded px-3 py-2" />
      </div>
      <div>
        <label class="block text-sm text-neutral-300 mb-1">Fat (g)</label>
        <input type="number" step="0.1" min="0" required name="fat_g" class="w-full bg-neutral-900 border border-neutral-800 rounded px-3 py-2" />
      </div>
    </div>
    <button type="submit" class="px-4 py-2 rounded-lg bg-brand text-white">Save</button>
  </form>
  <script>
  (function(){
    ...
    btn.addEventListener('click', async ()=>{
      ...
      btn.disabled = true; msg.textContent = 'Detecting...';
      try {
        ...
        if (!resp.ok){
          ...
          if (typeof data.fat_g !== 'undefined') f.value = Math.max(0, parseFloat(data.fat_g||0));
          msg.textContent = 'Values filled. Review and Save.';
        }
      } catch (e){
